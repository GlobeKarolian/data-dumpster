...
|---|---|---|---|
| `/api/cron/ingest` | `0 */3 * * *` (every 3h) | 300s | Eight runs a day is frequent enough that a post is in Pressbox within a news cycle, and infrequent enough to stay inside the YouTube Data API's 10,000 free units per day across a landscape of this size. Engagement on social is not immutable, so each run also refreshes recent posts; three hours is roughly the interval at which those numbers have moved enough to be worth another call. |
