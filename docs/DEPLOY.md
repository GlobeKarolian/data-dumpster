...
| TWITTER_BEARER_TOKEN | Primary pooled X source since 17 August 2026 (app-only, pay-per-use; see HANDOFF.md). When it is empty, X collects through Bright Data alone and has no other fallback, so a Bright Data outage takes X down with it. `/api/health` reports `spend.x: null` when this is unset. Watch spend in the X Developer Console; new-channel history backfills cost far more than steady-state refreshes |
