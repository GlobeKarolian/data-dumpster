/**
 * Instagram comment collection: buy the comments under our pooled posts.
 *
 * Why comments at all: the earliest signal of an emerging theme lives under
 * posts, not in them, and nobody can read fifty comment sections a day. This
 * is the first platform of the comment program; Instagram goes first because
 * its whole corpus runs about $2 a day, so the always-on question disappears
 * while the shape of the data gets learned.
 *
 ...
 * caps that bind (probed 25 Aug: asked 5, got exactly 5).
 *
...
import {
  IG_COMMENT_DAILY_RECORD_BUDGET, estimateBrightDataCents, recordSpend, remainingRecordBudget,
...
export function parseComment(postId: string, r: RawComment): {
  ...
  return {
    postId,
    externalId,
    authorName: str(r.comment_user),
    authorUrl: str(r.comment_user_url),
    text: str(r.comment),
    likes: int(r.likes_number),
    replies: int(r.replies_number),
    commentedAt: date(r.comment_date),
  };
}
 ...
 */
async function claimPosts(): Promise<{ id: string; permalink: string; resume: string | null }[]> {
...
export async function runCommentCollection(apiKey: string): Promise<CommentCollectResult> {
  ...
  let budget = await remainingRecordBudget(
    'brightdata', IG_COMMENT_DAILY_RECORD_BUDGET, 24, DATASETS.instagramComments,
  );
  if (budget < COMMENTS_PER_POST) {
    result.budgetExhausted = true;
    console.warn('[data-dumpster:comments] daily record budget exhausted, buying nothing', {
      budgetRemaining: budget, dailyBudget: IG_COMMENT_DAILY_RECORD_BUDGET,
    });
    return result;
  }

  const claimed = await claimPosts();
  ...
  for (const post of claimed) {
    ...
    try {
      ...
      await recordSpend({
        vendor: 'brightdata',
        resource: DATASETS.instagramComments,
      ...
      if (rows.length > COMMENTS_PER_POST * 2) {
        console.error('[data-dumpster:comments] vendor exceeded its own record cap', {
          post: post.permalink, asked: COMMENTS_PER_POST, delivered: rows.length,
        });
      }
