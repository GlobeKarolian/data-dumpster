...
import { ArrowLeft, CalendarDays, Check, ExternalLink, Link2, Plus, ShieldCheck, UserRound, X } from 'lucide-react';
...
function ProfileStatus({ source }: { source: ElectionCandidateSource }) {
  ...
  return <Badge tone={connected ? 'positive' : paused ? 'warning' : source.status === 'error' ? 'critical' : 'outline'}>{connected ? 'Connected' : paused ? 'Paused' : source.status === 'error' ? 'Needs attention' : 'Ready to verify'}</Badge>;
}
    ...
    <Card>
      ...
      <div className="divide-y divide-zinc-100 dark:divide-zinc-800/70">
        {candidate.sources.map((source) => {
          ...
          const mayConnect = canEdit && source.platform !== 'facebook' && !profile;
          return (
            <div key={source.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
              <PlatformIcon platform={source.platform} className="h-5 w-5" label />
              <div className="min-w-[12rem] flex-1">
                ...
                <a href={source.url} target="_blank" rel="noopener noreferrer" className="mt-0.5 block truncate text-[11px] text-zinc-500 hover:text-red-700 hover:underline">{profile ? '@' + profile.handle.replace(/^@/, '') : source.url}</a>
                {source.note ? <p className="mt-1 text-[10px] leading-4 text-amber-700 dark:text-amber-400">{source.note}</p> : null}
              </div>
              {profile ? <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-700"><Check className="h-3.5 w-3.5" /> {profile.lastIngestedAt ? 'Collecting' : 'Queued'}</span> : mayConnect ? <Button size="sm" onClick={() => setSelected(source)}>Verify & connect</Button> : null}
            </div>
          );
        })}
      </div>
      ...
      <Dialog open={selected !== null} onOpenChange={(open) => { if (!open) setSelected(null); }} labelledBy={'verify-' + candidate.id} className="max-w-3xl">
        {selected ? <><div className="flex items-start justify-between border-b border-zinc-200 px-5 py-4 dark:border-zinc-800"><div><h2 id={'verify-' + candidate.id} className="text-lg font-semibold">Verify {candidate.name} on {PLATFORM_LABELS[selected.platform]}</h2><p className="mt-1 text-xs text-zinc-500">The account returned by the live source must be reviewed before it joins the pooled dataset.</p></div><Button variant="ghost" size="icon" onClick={() => setSelected(null)}><X className="h-4 w-4" /></Button></div><div className="max-h-[70vh] overflow-y-auto p-5"><AddChannelForm companyId={candidate.companyId} existing={candidate.profiles.map((profile) => profile.platform)} preferredPlatform={selected.platform} initialInput={selected.url} onCancel={() => setSelected(null)} onDone={() => { setSelected(null); router.refresh(); }} /></div></> : null}
      </Dialog>
    </Card>
  );
}
...
export function ElectionRaceWorkspace({ race, canEdit, manualRefreshAllowed }: { race: ElectionRaceDetail; canEdit: boolean; manualRefreshAllowed: boolean }) {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><Link href="/elections" className="inline-flex items-center gap-1 text-xs font-medium text-zinc-500 hover:text-red-700"><ArrowLeft className="h-3.5 w-3.5" /> Election Center</Link><div className="mt-3 flex flex-wrap items-center gap-2"><h2 className="text-2xl font-semibold tracking-[-0.025em]">{race.name}</h2><Badge tone={race.status === 'active' ? 'positive' : 'warning'}>{race.status === 'active' ? 'Collecting' : 'Setup'}</Badge></div><p className="mt-1 flex items-center gap-1.5 text-sm text-zinc-500"><CalendarDays className="h-4 w-4" /> {formatDate(race.electionDate)} · {race.jurisdiction}</p></div><div className="flex flex-wrap gap-2"><RefreshButton landscapeId={race.landscapeId} manualRefreshAllowed={manualRefreshAllowed} className="w-auto" />{canEdit ? <Button variant="primary" onClick={() => setAdding(true)}><Plus className="h-4 w-4" /> Add candidate</Button> : null}</div></div>
    ...
    <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-xs leading-5 text-blue-900 dark:border-blue-900/60 dark:bg-blue-950/25 dark:text-blue-300"><strong>Campaign-only scope.</strong> Official Senate and House accounts are deliberately excluded. Supplied accounts do not enter rankings until the live source resolves them and a person confirms the match.</div>
